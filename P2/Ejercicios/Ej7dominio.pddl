(define (domain ej7-domain)
  ...
  (:action give
    :parameters (?d - dealer ?n - npc ?z - zone ?o - normal-obj)
    :precondition (and
      (hand ?d ?o)
      (at ?d ?z)
      (at ?n ?z)
    ...
    :effect (and
      ...
      (not (has-obj ?d))
      (has-obj ?n)
      (increase (total-points ?d) (points ?n ?o))
      (increase (pocket ?n) 1)
    )
  )

  (:action give-to-player
    :parameters (?p - picker ?d - dealer ?z - zone ?o - obj)
    :precondition (and
      (hand ?p ?o)
      (not (has-obj ?d))
      (at ?p ?z)
    ...
    :effect (and
      ...
      (has-obj ?d)
      (hand ?d ?o)
      (when (is-bikini ?o) (and (has-bikini ?d) (not (has-bikini ?p))))
      (when (is-sneakers ?o) (and (has-sneakers ?d) (not (has-sneakers ?p))))
    )
  )

  (:action pick-up
    :parameters (?p - picker ?o - obj ?z - zone)
    :precondition (and
      (at ?p ?z)
      (at ?o ?z)
      (not (has-obj ?p))
